...
ssh $USER_NAME@$HOST_NAME "cd ulapp && git fetch && git pull origin production && docker-compose down && docker-compose run app rails assets:precompile RAILS_ENV=production && docker-compose up -d && docker-compose run app rails db:migrate RAILS_ENV=production"