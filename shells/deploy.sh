...
ssh $USER_NAME@$HOST_NAME "cd ulapp && git pull origin production && docker-compose down && docker-compose up -d && docker-compose run app rails db:migrate RAILS_ENV=production"