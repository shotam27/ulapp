...
ssh $USER_NAME@$HOST_NAME "cd ulapp && git fetch && git pull origin production && docker-compose down && docker-compose up -d && docker-compose exec app rails db:migrate RAILS_ENV=production"